...
class GroceriesTableViewController: UITableViewController {
    ...
    var currentTasksList: GroceryList!
    private var currentTasks: Results<Groceries>!
    private var completedTasks: Results<Groceries>!
    ...
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        return section == 0 ? "Items You Need" : "In Cart"
    }
    ...
    private func filteringTasks() {
        currentTasks = currentTasksList.groceries.filter("isComplete = false")
        completedTasks = currentTasksList.groceries.filter("isComplete = true")
...
extension GroceriesTableViewController {
    private func alertForAddAndUpdateList(_ groceryName: Groceries? = nil) {
        ...
        let saveAction = UIAlertAction(title: doneButton, style: .default) { _ in
            ...
            if let groceryName = groceryName {
                if let newNote = noteTextField.text, !newNote.isEmpty {
                    StorageManager.editTask(groceryName, newTask: newTask, newNote: newNote)
                } else {
                    StorageManager.editTask(groceryName, newTask: newTask, newNote: "")
                }
        ...
        alert.addTextField { textField in
            ...
            if let groceryName = groceryName {
                noteTextField.text = groceryName.note
            }
        }
