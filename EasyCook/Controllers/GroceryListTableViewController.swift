...
class GroceryListTableViewController: UITableViewController {
    ...
    var groceryLists: Results<GroceryList>!
    private var isEditingMode = false
    
    override func viewDidLoad() {
        super.viewDidLoad()

        groceryLists = realm.objects(GroceryList.self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let indexPath = tableView.indexPathForSelectedRow {
            ...
            let tasksVC = segue.destination as! GroceriesTableViewController
            tasksVC.currentTasksList = groceryList
        }
        if segue.identifier == "groceryInfo" {
            // Get the new view controller using segue.destination.
                let vc = segue.destination as! AppInfoViewController
            // Pass the selected object to the new view controller.
                vc.infoText = "Here you can make and manage your grocery list!"
        }
    }
...
//        cell.textLabel?.text = tasksList.name
        ...
        cell.configure(with: groceryList)
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration?  {
        ...
        let currentList = groceryLists[indexPath.row]
...
extension GroceryListTableViewController {
    ...
    private func alerForAddAndUpdateList(_ listName: GroceryList? = nil, complition: (() -> Void)? = nil) {
        ...
        let saveAction = UIAlertAction(title: doneButton, style: .default) { _ in
            ...
            if let listName = listName {
                ...
                groceryList.name = newList
                
                StorageManager.saveGroceryList(groceryList)
                self.tableView.insertRows(at: [IndexPath(row: self.groceryLists.count - 1, section: 0)], with: .automatic
                )
            }//else
        }
...
extension UITableViewCell {
    ...
    func configure(with groceryList: GroceryList) {
        let currentTasks = groceryList.groceries.filter("isComplete = false")
        let completedTasks = groceryList.groceries.filter("isComplete = true")
        
        textLabel?.text = groceryList.name
